...
HandleData.prototype.verify = function(){
    let verifyNumber = this.data.slice(-1).toString();
    let sum = 0,sumNumber;
...
HandleData.prototype.mainData = function(){
    ...
    let handleTime = (function(t){
    let Year = (Array(4).join('0') + parseInt(this.mData[t] + this.mData[++t],16)).slice(-4);
    let Month = (Array(2).join('0') + parseInt(this.mData[++t],16)).slice(-2);
    let Day = (Array(2).join('0') + parseInt(this.mData[++t],16)).slice(-2);
    ++t;
    let Hour = (Array(2).join('0') + parseInt(this.mData[++t],16)).slice(-2);
    let Minute = (Array(2).join('0') + parseInt(this.mData[++t],16)).slice(-2);
    let Second = (Array(2).join('0') + parseInt(this.mData[++t],16)).slice(-2);
    ...
    if(2 <= dataFrames.alarmType <= 18){
        switch(dataFrames.alarmType){
            ...
            case 5:
                this.eventCode = 'M02';
                break;
            case 6 <= dataFrames.alarmType <= 14:
    ...
    let readCardVer = parseInt(this.mData[++i],16);
    let a = parseInt(readCardVer / 10);
    let b = readCardVer % 10;
...
function Bdecode(codes,n){
    let value = (Array(n).join('0') + parseInt(codes.join(''),16)).slice(-n);
    return value;
}
//解码ASCII
...
function dateDecode(codes){
    let t=0;
    let Year = (Array(4).join('0') + parseInt(codes[t] + codes[++t],16)).slice(-4);
    let Month = (Array(2).join('0') + parseInt(codes[++t],16)).slice(-2);
    let Day = (Array(2).join('0') + parseInt(codes[++t],16)).slice(-2);
    ++t;
    let Hour = (Array(2).join('0') + parseInt(codes[++t],16)).slice(-2);
    let Minute = (Array(2).join('0') + parseInt(codes[++t],16)).slice(-2);
    let Second = (Array(2).join('0') + parseInt(codes[++t],16)).slice(-2);
    let time = Year + "-" + Month + "-" + Day + " " + Hour + ":" + Minute + ":" + Second;
    return time;
}
HandleData.prototype.coinData = function(){
    ...
    let cData = this.mData.slice(64,-1);
    let n=0;
    dataFrames.coinData = [];
